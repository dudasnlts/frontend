<template>
  <AdminLayout>
    <div class="page-head">
      <div><h1>Joias</h1><p>Gerencie o catálogo de joias</p></div>
      <button class="add-btn" @click="openModal()">+ Nova Joia</button>
    </div>

    <div v-if="loading" class="loading">Carregando...</div>
    <div v-else-if="items.length === 0" class="empty">Nenhuma joia cadastrada.</div>
    <div v-else class="grid">
      <div class="card" v-for="item in items" :key="item._id">
        <img v-if="item.image" :src="imgUrl(item.image)" alt="" />
        <div class="no-img" v-else><i class="bi bi-gem"></i></div>
        <div class="card-body">
          <h3>{{ item.name }}</h3>
    ...
    <div v-if="showModal" class="modal-overlay" @click.self="closeModal">
      <div class="modal">
        <div class="modal-head">
          <h2>{{ editing ? 'Editar Joia' : 'Nova Joia' }}</h2>
